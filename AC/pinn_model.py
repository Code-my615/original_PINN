...
class BasePINN:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        self.pde_lbd = params["pde_params"]["lbd"]
        self.pde_epsilon = params["pde_params"]["epsilon"]
...
class AC_forward(BasePINN):
    ...
    def __init__(self, params):
        ...
        # 将边界数据分成x,t. 目的为了之后自动微分
        self.X_lbc_x = torch.tensor(params["train_data"]["left_boundary"][:, 0].reshape(-1, 1), dtype=torch.float, device=self.device, requires_grad=True)
        self.X_lbc_t = torch.tensor(params["train_data"]["left_boundary"][:, 1].reshape(-1, 1), dtype=torch.float, device=self.device, requires_grad=True)
        self.X_rbc_x = torch.tensor(params["train_data"]["right_boundary"][:, 0].reshape(-1, 1), dtype=torch.float, device=self.device, requires_grad=True)
        self.X_rbc_t = torch.tensor(params["train_data"]["right_boundary"][:, 1].reshape(-1, 1), dtype=torch.float, device=self.device, requires_grad=True)
    ...
    def residual_net(self, x, t):
        ...
        f = u_t - self.pde_lbd * u_xx + self.pde_epsilon * (u ** 3 - u)
        return f

    def bc_x_net(self, X_lbc_x, X_lbc_t, X_rbc_x, X_rbc_t):
        X_lbc = torch.cat((X_lbc_x, X_lbc_t), dim=1)
        X_rbc = torch.cat((X_rbc_x, X_rbc_t), dim=1)
        U_pre_lbc = self.neural_net(X_lbc)
        U_pre_rbc = self.neural_net(X_rbc)
        u_lbc_x = torch.autograd.grad(
            U_pre_lbc, X_lbc_x,
            grad_outputs=torch.ones_like(U_pre_lbc),
            retain_graph=True,
            create_graph=True,
            allow_unused=True
        )[0]
        u_rbc_x = torch.autograd.grad(
            U_pre_rbc, X_rbc_x,
            grad_outputs=torch.ones_like(U_pre_rbc),
            retain_graph=True,
            create_graph=True,
            allow_unused=True
        )[0]
        return u_lbc_x, u_rbc_x
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            loss_bc1 = torch.mean((u_pre_lbc - u_pre_rbc) ** 2)
            # # 边界条件2
            u_pre_lbc_x, u_pre_rbc_x = self.bc_x_net(self.X_lbc_x, self.X_lbc_t, self.X_rbc_x, self.X_rbc_t)
            loss_bc2 = torch.mean((u_pre_lbc_x - u_pre_rbc_x) ** 2)
            ...
            if (epoch+1) % 1000 == 0:
                # print("[nu: %d], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_lbd, (epoch+1), loss, self.l2_error))
                print("[lbd: %.6f, epsilon: %.6f], Epoch: %d ,loss: %.16f" % (self.pde_lbd, self.pde_epsilon, (epoch + 1), loss))
                # plt.figure(figsize=(5, 4), dpi=150)
                # plt.pcolor(self.TT, self.XX, u_pred.reshape(self.n_x, self.n_t), cmap='jet')
                # plt.colorbar()
                # plt.xlabel('$t$')
                # plt.ylabel('$x$')
                # plt.savefig(os.path.join(self.base_dir, f"pre_u_solution_epoch_{epoch}.png"))
                # plt.close()
...
class AC_inverse(BasePINN):

    def __init__(self, params):
        super().__init__(params)
        # inverse
        # 随机选择 0 或 1，来从 self.pde_lbd + 2 和 self.pde_lbd-2 中选择
        random_index = torch.randint(0, 2, (1,)).item()
        train_lbd = (self.pde_lbd + 0.1) if random_index == 0 else (self.pde_lbd - 0.1)
        train_epsilon = (self.pde_epsilon + 1) if random_index == 0 else (self.pde_epsilon - 1)
        # 初始化pde参数
        self.train_lbd = torch.nn.Parameter(torch.tensor(train_lbd).float().to(self.device))
        self.train_epsilon = torch.nn.Parameter(torch.tensor(train_epsilon).float().to(self.device))
        self.optimizer = self.optimizer_fn(list(self.dnn.parameters()) + [self.train_lbd] + [self.train_epsilon], self.lr)
    ...
    def residual_net(self, x, t):
        ...
        f = u_t - self.train_lbd * u_xx + self.train_epsilon * (u**3 - u)
        return f
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            self.lbd_log.append(self.train_lbd.detach().item())
            self.epsilon_log.append(self.train_epsilon.detach().item())

            MAE_lbd = np.abs(self.train_lbd.cpu().detach().numpy() - self.pde_lbd).mean()
            MSE_lbd = ((self.train_lbd.cpu().detach().numpy() - self.pde_lbd) ** 2).mean()
            l1_error_lbd = MAE_lbd / np.abs(self.pde_lbd).mean()
            l2_error_lbd = np.linalg.norm(self.train_lbd.cpu().detach().numpy() - self.pde_lbd) / np.linalg.norm(self.pde_lbd)

            MAE_epsilon = np.abs(self.train_epsilon.cpu().detach().numpy() - self.pde_epsilon).mean()
            MSE_epsilon = ((self.train_epsilon.cpu().detach().numpy() - self.pde_epsilon) ** 2).mean()
            l1_error_epsilon = MAE_lbd / np.abs(self.pde_epsilon).mean()
            l2_error_epsilon = np.linalg.norm(self.train_epsilon.cpu().detach().numpy() - self.pde_epsilon) / np.linalg.norm(self.pde_epsilon)

            self.MAE = (MAE_lbd + MAE_epsilon)/2
            self.MSE = (MSE_lbd + MSE_epsilon)/2
            self.l1_error = (l1_error_lbd + l1_error_epsilon)/2
            self.l2_error = (l2_error_lbd + l2_error_epsilon)/2

            self.abs_err_log.append(self.l1_error)
            self.rel_l2_err_log.append(self.l2_error)

            if (epoch + 1) % 1000 == 0:
                print("[[real_lbd: %.6f, real_epsilon: %.6f], [predicted_lbd: %.6f, predicted_epsilon: %.6f]], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_lbd, self.pde_epsilon,
                                            self.train_lbd, self.train_epsilon,(epoch+1), loss, self.l2_error))
