...
parser.add_argument('--experiment', type=str, default='AC')
...
parser.add_argument('--t_num', type=int, default=101, help='Number of t-coordinates for ground truth grid')  # 测试t轴取的点数
parser.add_argument('--range_x', type=float, nargs=2, default=[-1, 1])
parser.add_argument('--range_t', type=float, nargs=2, default=[0, 1])
parser.add_argument('--lbd', type=int, default=0.0001)  # AC lambda系数
parser.add_argument('--epsilon', type=int, default=5)  # AC epsilon系数
...
if args.problem == 'forward':
    sub_dir = f"{args.problem}/lbd={args.lbd}_epsilon={args.epsilon}/col_points_{args.col_points},ic_points_{args.ic_points},bc_points_{args.bc_points}/lr={args.lr}/epochs_{args.epochs}"
elif args.problem == 'inverse':
    sub_dir = f"{args.problem}/lbd={args.lbd}_epsilon={args.epsilon}/col_points_{args.col_points}_usol_num_{args.usol_num}/epochs_{args.epochs}"
else:
    print("输入有误！")
    sys.exit()
...
# AC真实解的路径
usol_npy_path = f'./test_dataset/lbd={args.lbd}_epsilon={args.epsilon}/usol.npy'
usol = np.load(usol_npy_path)
...
if args.problem == 'forward':
    model = AC_forward(params)
else:
    model = AC_inverse(params)
