...
class BasePINN:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        self.pde_lambda_1 = params["pde_params"]["lambda_1"]
        self.pde_lambda_2 = params["pde_params"]["lambda_2"]
        ...
        self.XX, self.YY = np.meshgrid(x_star, y_star)
    ...
    def neural_net(self, X):
        ...
        return u.reshape(-1, 1), v.reshape(-1, 1), p.reshape(-1, 1)
    ...
    def loss_pde(self):
        pde_f, pde_g, pde_h = self.residual_net(self.X_f_x, self.X_f_y)
        loss_pde_f = torch.mean(pde_f**2)
        loss_pde_g = torch.mean(pde_g ** 2)
        loss_pde_h = torch.mean(pde_h ** 2)
        return loss_pde_f + loss_pde_g + loss_pde_h
...
class NS_forward(BasePINN):
    ...
    def __init__(self, params):
        ...
        # 取出边界条件点
          # 管壁
        self.X_pipe_wall = torch.tensor(params["train_data"]["pipe_wall"]["X_pipe_wall"], dtype=torch.float, device=self.device)
        self.u_pipe_wall = torch.tensor(params["train_data"]["pipe_wall"]["u_pipe_wall"], dtype=torch.float, device=self.device)
        self.v_pipe_wall = torch.tensor(params["train_data"]["pipe_wall"]["v_pipe_wall"], dtype=torch.float, device=self.device)
          # 管道入口
        self.X_left_inlet = torch.tensor(params["train_data"]["inlet"]["X_left_inlet"], dtype=torch.float, device=self.device)
        self.u_left_inlet = torch.tensor(params["train_data"]["inlet"]["u_inlet"], dtype=torch.float, device=self.device)
        self.v_left_inlet = torch.tensor(params["train_data"]["inlet"]["v_inlet"], dtype=torch.float, device=self.device)
          # 管道出口
        self.X_right_outlet = torch.tensor(params["train_data"]["outlet"]["X_right_outlet"], dtype=torch.float, device=self.device)
        self.p_outlet = torch.tensor(params["train_data"]["outlet"]["p_outlet"], dtype=torch.float,device=self.device)
    ...
    def residual_net(self, x, y):
        ...
        p_x = torch.autograd.grad(p, x,  grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]
        p_y = torch.autograd.grad( p, y, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]

        u_y = torch.autograd.grad(u, y, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]
        u_yy = torch.autograd.grad(u_y, y, grad_outputs=torch.ones_like(u), retain_graph=True, create_graph=True)[0]

        f = self.pde_lambda_1 * (u * u_x + v * u_y) + p_x + self.pde_lambda_2 * (u_xx + u_yy)
        g = self.pde_lambda_1 * (u * u_x + v * u_y) + p_y + self.pde_lambda_2 * (u_xx + u_yy)
        h = u_x + u_y
        return f, g, h
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            u_pipe_wall_pre, v_pipe_wall_pre, p_pipe_wall_pre = self.neural_net(self.X_pipe_wall)
            loss_u_pipe_wall = torch.mean((u_pipe_wall_pre - self.u_pipe_wall) ** 2)
            loss_v_pipe_wall = torch.mean((v_pipe_wall_pre - self.v_pipe_wall) ** 2)
            loss_pipe_wall = loss_u_pipe_wall + loss_v_pipe_wall

            u_left_inlet_pre, v_left_inlet_pre, p_left_inlet_pre = self.neural_net(self.X_left_inlet)
            loss_u_left_inlet = torch.mean((u_left_inlet_pre - self.u_left_inlet) ** 2)
            loss_v_left_inlet = torch.mean((v_left_inlet_pre - self.v_left_inlet) ** 2)
            loss_inlet = loss_u_left_inlet + loss_v_left_inlet

            u_right_outlet_pre, v_right_outlet_pre, p_right_outlet_pre = self.neural_net(self.X_right_outlet)
            loss_outlet = torch.mean((p_right_outlet_pre - self.p_outlet) ** 2)

            loss_bc = loss_pipe_wall + loss_inlet + loss_outlet

            loss = self.lambda_pde * self.loss_pde() + self.lambda_bc * loss_bc
            ...
            if (epoch+1) % 1000 == 0:
                ...
                print("[lambda1: %.6f, lambda2： %.6f], Epoch: %d ,loss: %.16f" % (self.pde_lambda_1, self.pde_lambda_2, (epoch + 1), loss))
