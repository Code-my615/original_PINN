...
class BasePINN:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        self.TT, self.XX = np.meshgrid(t_star, x_star)  # all the X grid points T times, all the T grid points X times
...
class Burgers_forward(BasePINN):
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            if (epoch+1) % 1000 == 0:
                # print("[nu: %d], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_nu, (epoch+1), loss, self.l2_error))
                print("[nu: %f], Epoch: %d ,loss: %.16f" % (self.pde_nu, (epoch + 1), loss))
...
class Burgers_inverse(BasePINN):

    def __init__(self, params):
        super().__init__(params)
        # inverse
        # 随机选择 0 或 1，来从 self.pde_nu + 2 和 self.pde_nu-2 中选择
        random_index = torch.randint(0, 2, (1,)).item()
        train_nu = (self.pde_nu + 0.1) if random_index == 0 else (self.pde_nu - 0.1)
        # 初始化pde参数
        self.train_nu = torch.nn.Parameter(torch.tensor(train_nu).float().to(self.device))
        self.optimizer = self.optimizer_fn(list(self.dnn.parameters()) + [self.train_nu], self.lr)
    ...
    def residual_net(self, x, t):
        ...
        f = u_t + u * u_x - self.train_nu * u_xx
        return f
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            self.nu_log.append(self.train_nu.detach().item())
            self.MAE = np.abs(self.train_nu.cpu().detach().numpy() - self.pde_nu).mean()
            self.MSE = ((self.train_nu.cpu().detach().numpy() - self.pde_nu) ** 2).mean()
            self.l1_error = self.MAE / np.abs(self.pde_nu).mean()
            self.l2_error = np.linalg.norm(self.train_nu.cpu().detach().numpy() - self.pde_nu) / np.linalg.norm(self.pde_nu)

            self.abs_err_log.append(self.l1_error)
            self.rel_l2_err_log.append(self.l2_error)

            if (epoch + 1) % 1000 == 0:
                print("[real_nu: %f, Predicted nu: %.6f], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_nu, self.train_nu, (epoch+1), loss, self.l2_error))
