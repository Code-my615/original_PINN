...
parser.add_argument('--t_num', type=int, default=100, help='Number of t-coordinates for ground truth grid')  # 测试t轴取的点数
...
parser.add_argument('--u0_str', type=str, default='sin(2*x)')  # 对流方程中的初始条件
...
if args.problem == 'forward':
    sub_dir = f"{args.problem}/beta={args.beta}/ic_function={safe_u0_str}/col_points_{args.col_points},ic_points_{args.ic_points},bc_points_{args.bc_points}/lr={args.lr}/epochs_{args.epochs}"
elif args.problem == 'inverse':
    sub_dir = f"{args.problem}/beta={args.beta}/ic_function={safe_u0_str}/col_points_{args.col_points}_usol_num_{args.usol_num}/epochs_{args.epochs}"
else:
    print("输入有误！")
    sys.exit()
...
dataset = Dataset(args.range_x[0], args.range_t[0], args.range_x[1], args.range_t[1], args.col_points, args.bc_points, args.ic_points, args.u0_str, args.problem, args.seed)
...
usol = convection_diffusion(args.u0_str, 0.0, args.beta,
                              ...
                              xgrid=args.x_num, nt=args.t_num)
...
if args.problem == 'forward':
    model = Convection_forward(params)
else:
