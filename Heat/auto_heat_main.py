...
from gen_exact_heat import gen_heat_exact_solution


parser = argparse.ArgumentParser(description='take params')
parser.add_argument('--experiment', type=str, default='heat')
...
parser.add_argument('--alpha', type=int, default=6)  # 对流方程系数
...
for args.alpha in [5, 8, 10, 13, 15, 19]:
    # 创建目录
    save_dir = f"Seed_{args.seed}_{args.experiment}"
    # os.makedirs(save_dir, exist_ok=True)
    # 创建子目录
    if args.problem == 'forward':
        sub_dir = f"{args.problem}/alpha={args.alpha}/col_points_{args.col_points},ic_points_{args.ic_points},bc_points_{args.bc_points}/lr={args.lr}/epochs_{args.epochs}"
    else:
        sub_dir = f"{args.problem}/alpha={args.alpha}/col_points_{args.col_points}_usol_num_{args.usol_num}/epochs_{args.epochs}"
    ...
    usol = gen_heat_exact_solution(args.x_num, args.t_num, args.n, args.alpha, x_upper_bound=args.range_x[1], t_upper_bound=args.range_t[1])
    ...
    if args.problem == 'forward':
        model = Heat_forward(params)
    elif args.problem == 'inverse':
        model = Heat_inverse(params)
    else:
        print("输入有误！")
        sys.exit()
    ...
    np.savez(os.path.join(base_dir, f"alpha={args.alpha}_u_pre.npz"), t=t_star, x=x_star, u_pre=u_pred_picture)
    print(l2_error)
    # predict solution
    plt.figure(figsize=(5, 4), dpi=150)
