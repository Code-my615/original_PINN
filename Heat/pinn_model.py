...
class BasePINN:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        # self.X_f_x.requires_grad = True
        # self.X_f_t.requires_grad = True
...
class Heat_forward(BasePINN):
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            self.loss_pde_log.append(loss.item())
...
class Heat_inverse(BasePINN):

    def __init__(self, params):
        super().__init__(params)
        # inverse
        # 随机选择 0 或 1，来从 self.pde_alpha + 2 和 self.pde_alpha-2 中选择
        random_index = torch.randint(0, 2, (1,)).item()
        train_alpha = (self.pde_alpha + 2) if random_index == 0 else (self.pde_alpha - 2)
        # 初始化pde参数
        self.train_alpha = torch.nn.Parameter(torch.tensor(train_alpha).float().to(self.device))
        self.optimizer = self.optimizer_fn(list(self.dnn.parameters()) + [self.train_alpha], self.lr)
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            self.alpha_log.append(self.train_alpha.detach().item())
            self.MAE = np.abs(self.train_alpha.cpu().detach().numpy() - self.pde_alpha).mean()
            self.MSE = ((self.train_alpha.cpu().detach().numpy() - self.pde_alpha) ** 2).mean()
            self.l1_error = self.MAE / np.abs(self.pde_alpha).mean()
            self.l2_error = np.linalg.norm(self.train_alpha.cpu().detach().numpy() - self.pde_alpha) / np.linalg.norm(self.pde_alpha)

            self.abs_err_log.append(self.l1_error)
            self.rel_l2_err_log.append(self.l2_error)

            if (epoch + 1) % 1000 == 0:
                print("[real_alpha: %d, Predicted_alpha: %.6f], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_alpha, self.train_alpha, (epoch+1), loss, self.l2_error))
