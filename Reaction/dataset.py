...
class Dataset:
    ...
    def _generate_points(self):
        ...
        zeros = np.zeros(self.boundaries_num)
        self.left_boundary = np.column_stack((zeros, t_values))
        ...
        self.ic_y_true = np.exp(-(self.initial[:, 0] - np.pi) ** 2 / (2 * (np.pi / 4) ** 2)).reshape(-1, 1)
