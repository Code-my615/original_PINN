...
class BasePINN:
    def __init__(self, params):
       ...
       # Use optimizers to set optimizer initialization and update functions
        ...
        self.pde_ruo = params["pde_params"]["ruo"]
...
class Reaction_forward(BasePINN):
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            if (epoch+1) % 1000 == 0:
                # print("[ruo: %d], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_ruo, (epoch+1), loss, self.l2_error))
                print("[ruo: %.6f], Epoch: %d ,loss: %.16f" % (self.pde_ruo, (epoch + 1), loss))
...
class Reaction_inverse(BasePINN):

    def __init__(self, params):
        super().__init__(params)
        # inverse
        # 随机选择 0 或 1，来从 self.pde_ruo + 2 和 self.pde_ruo-2 中选择
        random_index = torch.randint(0, 2, (1,)).item()
        train_ruo = (self.pde_ruo + 2) if random_index == 0 else (self.pde_ruo - 2)
        # 初始化pde参数
        self.train_ruo = torch.nn.Parameter(torch.tensor(train_ruo).float().to(self.device))
        self.optimizer = self.optimizer_fn(list(self.dnn.parameters()) + [self.train_ruo], self.lr)
    ...
    def train(self, epochs):
        ...
        for epoch in pbar:
            ...
            self.ruo_log.append(self.train_ruo.detach().item())
            self.MAE = np.abs(self.train_ruo.cpu().detach().numpy() - self.pde_ruo).mean()
            self.MSE = ((self.train_ruo.cpu().detach().numpy() - self.pde_ruo) ** 2).mean()
            self.l1_error = self.MAE / np.abs(self.pde_ruo).mean()
            self.l2_error = np.linalg.norm(self.train_ruo.cpu().detach().numpy() - self.pde_ruo) / np.linalg.norm(self.pde_ruo)

            self.abs_err_log.append(self.l1_error)
            self.rel_l2_err_log.append(self.l2_error)

            if (epoch + 1) % 1000 == 0:
                print("[real_ruo: %.6f, Predicted ruo: %.6f], Epoch: %d ,loss: %.16f, l2_error: %.16f" % (self.pde_ruo, self.train_ruo, (epoch+1), loss, self.l2_error))
